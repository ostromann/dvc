...
def _show_mermaid(graph, markdown: bool = False, status: bool = False):
    ...
    for pipeline in pipelines:
        ...
        for node in nodes:
            total_nodes += 1
            node_id = f"node{total_nodes}"  # if status, set node classes
            graph += f'\n\t{node_id}["{node}"]'
    ...
    if rsc_list:
        ...
        for rsc in rsc_list:
            ...
            if str(rsc) in cloud_status:
                rsc_pushed = False
                all_rsc_pushed = False

            pipeline.nodes[stage.addressing][rsc_key][rsc_name] = {
                "clean": rsc_clean,
                "pushed": rsc_pushed,
            }

        pipeline.nodes[stage.addressing][rsc_key + " clean"] = all_rsc_clean
        pipeline.nodes[stage.addressing][rsc_key + " pushed"] = all_rsc_pushed
...
def add_parser(subparsers, parent_parser):
    ...
    dag_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Check the status of stages in the DAG. (Only compatible with --mermaid)",
    )
    dag_parser.add_argument(
        "--status-import",
        action="store_true",
